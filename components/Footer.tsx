import Link from 'next/link'
import Image from 'next/image'

export default function Footer() {
  return (
    <footer className="footer">
      <div className="container">
        <div className="footer__grid">
          <div className="footer__brand">
            <Image src="/mima-groups-darkuse.png" alt="MIMA Group" width={160} height={42} />
            <p>An integrated construction group. Karachi, Pakistan. Est. 1959.</p>
          </div>
          <div>
            <h4>Quick Links</h4>
            <ul>
              <li><Link href="/about">About</Link></li>
              <li><Link href="/#companies">MIMA Group</Link></li>
              <li><Link href="/portfolio">Portfolio</Link></li>
              <li><Link href="/clients">Clients</Link></li>
              <li><Link href="/#contact">Contact</Link></li>
            </ul>
          </div>
          <div>
            <h4>Our Companies</h4>
            <ul>
              <li><Link href="/mima-groups/electrical">MIMA Brothers</Link></li>
              <li><Link href="/mima-groups/construction">MIMA Construction</Link></li>
              <li><Link href="/mima-groups/facility">MIMA Facilities Management</Link></li>
              <li><Link href="/mima-groups/engineering">MIMA Engineering</Link></li>
            </ul>
          </div>
          <div className="footer__contact">
            <h4>Contact</h4>
            <p>Central Hotel Building<br />Civil Lines, Karachi</p>
            <p>+92 21 35685645<br />+92 21 35220466</p>
            <p><a href="mailto:info@mima.pk">info@mima.pk</a></p>
            <a
              href="https://www.linkedin.com/company/mima-construction-pk/"
              target="_blank"
              rel="noopener noreferrer"
              className="footer__linkedin"
              aria-label="MIMA Group on LinkedIn"
            >
              <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
                <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 0 1-2.063-2.065 2.064 2.064 0 1 1 2.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/>
              </svg>
              LinkedIn
            </a>
          </div>
        </div>
        <div className="footer__bar">
          <span>© 2026 MIMA Group · All rights reserved</span>
          <span>
            <a href="#">Privacy Policy</a>
            &nbsp;·&nbsp;
            <a href="#">Terms</a>
          </span>
        </div>
      </div>
    </footer>
  )
}
